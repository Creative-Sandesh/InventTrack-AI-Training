...
def get_category(
    ...
    cat = (
        ...
        .filter(models.Category.id == cat_id, models.Inventory.owner_id == owner_id)
        .first()
    )
    if not cat:
        return None
...
def update_category(
    db: Session, inv_id: str, cat_id: str, data: schemas.CategoryUpdate, owner_id: str
) -> Optional[schemas.CategoryOut]:
    cat = (
        db.query(models.Category)
        .join(models.Inventory, models.Category.inventory_id == models.Inventory.id)
        .filter(
            ...
            models.Category.inventory_id == inv_id,
...
def delete_category(
    db: Session, inv_id: str, cat_id: str, owner_id: str
) -> bool:
    cat = (
        db.query(models.Category)
        .join(models.Inventory, models.Category.inventory_id == models.Inventory.id)
...
def create_item(
    ...
    # Verify category belongs to the owner
    cat = (
        db.query(models.Category)
        .join(models.Inventory, models.Category.inventory_id == models.Inventory.id)
        .filter(
...
def get_dashboard_stats(db: Session, owner_id: str) -> schemas.DashboardStats:
    # Subquery: item ids owned by user
    owned_items_query = (
        db.query(models.InventoryItem)
        .join(models.Category, models.InventoryItem.category_id == models.Category.id)
        .join(models.Inventory, models.Category.inventory_id == models.Inventory.id)
    ...
    all_items = owned_items_query.all()
